...
| `2` | Weekly | По расписанию с учётом дня недели |
| `3` | Daily | По расписанию каждый день (день недели игнорируется) |
...
**Пример — перевести освещение в дневное расписание:**
```json
{
  "device_id":  "A0001",
  "message_id": "msg-002",
...
| Часть | Описание |
|-------|----------|
| `DD` | День недели: `00`=любой, `01`=пн, `02`=вт, `03`=ср, `04`=чт, `05`=пт, `06`=сб, `07`=вс |
| `hh` | Час (00–23) |
| `mm` | Минута (00–59) |

> В режиме **Daily** (`mode: 3`) поле `DD` игнорируется — диапазон повторяется каждый день.
...
### Нагреватель: поддерживать 22°C с пн по пт с 6:00 до 22:00
...
### Полив: каждый день 5 минут в 8:00 на 70% мощности
...
```json
{
  ...
  "schedule_unit": {
    ...
    "kind": "power",
    "interval": { "start": "00T08:00", "end": "00T08:05" },
...
```json
{
  "device_id": "A0001", "message_id": "msg-012",
  "type": "waterPump", "mode": 3
}
```
```json
{
  "device_id": "A0001", "message_id": "msg-013",
  "type": "humidifier", "mode": 0
}
```
